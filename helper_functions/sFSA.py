...
def run_sfsa(
    ...
    binary_word = ["0", "1"] * (word_length // 2)

    true_state_transitions = [fsa["S"][0]]  # this FSM starts at 'A'

    for dig in binary_word:  # computing true state transitions
        true_state_transitions.append(
            compute_transition(true_state_transitions[-1], dig, fsa)
        )
    ...
    _f = os.path.join(
        sFSA_model.data_folder, "simulation_summary.txt"
    ...
    sFSA_model.exportSfsaData(
        network_plot=save_plot, pickle_dump=False
    )  # computes state transitions

    if save_plot and save_path != "":
        title_addition = f"BA {ba:.2f} Hz, E-E max {e_e_maxw:.2f} mV, E-I {e_i_w:.2f} mV, I-E {i_e_w:.2f} mV, I {i_frequency:.2f} Hz"
    ...
    accuracy = sFSA_model.computeAccuracy(
        true_state_transitions, pred_state_transitions
    )
    ...
    return params | {
        "score": accuracy,
        "true_states": true_state_transitions,
        "pred_states": pred_state_transitions,
    }
...
class sFSA:
    def __init__(
        ...
        self.start_twindow = (
            0.2,
            0.1,
        )  # 1st: cuing period; 2nd: free (only for starting state).
        self.input_twindow = (
            params["cue_length"],
            params["cue_length"] + params["delay_A2GO"] + 1,
        )
    ...
    def __loadSfsaArgs(self):
        ...
        self.__RCN.delay_A2B = (
            self.__params["cue_length"] - self.__params["delay_gap_A2B"]
        ) * second
    ...
    def __loadSfsaModel(self):
        ...
        self.__sfsa_state["N_e"] = int(
            (len(self.__FSA_model["S"]) + len(self.__FSA_model["I"]))
            * self.attractor_size
        )
    ...
    def __initRcn4Fsa(self):
        ...
        self.__RCN.set_stimulus_i(
            stimulus="flat_to_I", frequency=self.__params["i_frequency"] * Hz
        )
    ...
    def __setStateTransitions(self):
        ...
        for tid, transition in self.__sfsa_state["T"].items():
            ...
            _s0 = (
                transition.split("->")[0].split(", ")[0].replace("(", "")
            )  # current state.
            _i = (
                transition.split("->")[0].split(", ")[1].replace(")", "")
            )  # input token.
            _s1 = transition.split("->")[1]  # next state.

            # - Setting connections for state->input->state transitions (transfering cue).

            self.__RCN.set_synapses_A_2_B(
                A_ids=self.__sfsa_state["I"][_i],
                B_ids=self.__sfsa_state["S"][_s0],
            ...
            if self.__sfsa_state["S"][_s0] != self.__sfsa_state["S"][_s1]:
                # if _i != '0' and _s1 != 'B':
                self.__RCN.set_synapses_A_2_GO(
                    A_ids=self.__sfsa_state["I"][_i],
                    GO_ids=self.__sfsa_state["S"][_s1],
                    weight=self.__params["w_acpt"] * mV,
                )

                # if _s0 != 'A' and _s1 != 'B':
                self.__RCN.set_synapses_A_2_GO(
                    A_ids=self.__sfsa_state["S"][_s0],
                    GO_ids=self.__sfsa_state["S"][_s1],
                    weight=self.__params["w_acpt"] * mV,
                )
    ...
    def __getMostActiveAttractor(self, spikes_per_state, t_window, attractor_size):
        ...
        for state, spikes in spikes_per_state.items():
            ...
            spks_in_window = [
                i for i in spikes if i >= t_window[0] and i <= t_window[1]
            ]
            ...
            if len(spks_in_window) > 0:
                ...
                if not found:
                    ...
                    try:
                        self.__RCN.stimulus_neurons_e_ids = self.__sfsa_state["I"][
                            state
                        ]
                    except:
                        pass
                ...
                states_mean_rate[state] = len(
                    np.argwhere(
                        (np.array(spk_mon_ts) >= t_window[0])
                        & (np.array(spk_mon_ts) <= t_window[1])
                    )
        ...
        if len(activations) == 0:
            ...
            if (
                max(zip(states_mean_rate.values(), states_mean_rate.keys()))[0]
                > rate_thr
            ):  # check if winning state is not at spontaneous activity level.
    ...
    def exportSfsaData(
        self, network_plot=True, pickle_dump=True, sub_dir="", name_ext=""
    ):
        """
        ...
        _E_spk_trains = {
            key: value for key, value in self.__RCN.E_mon.spike_trains().items()
        ...
        if pickle_dump:
            fn = os.path.join(
                self.data_folder, f"sSFA_simulation_data{name_ext}.pickle"
            )
    ...
    def computeStateHistory(self, data_path=""):
        ...
        if data_path != "":
            ...
            data_file = open(
                os.path.join(data_path, "sSFA_simulation_data.pickle"), "rb"
            )
        ...
        for S, S_neur_ids in simulation_data["sFSA"][
            "S"
        ].items():  # for each neuron in an attractor representing a state.

            attractor_size = len(
                S_neur_ids
            )  # assumes all attractors have the same size.

            spikes_per_state[S] = []  # all state's S spikes.

            for neur_id in simulation_data[
                "E_spk_trains"
            ].keys():  # for each neuron spike train.

                if (
                    neur_id in S_neur_ids
                ):  # save spikes if neuron belongs to attractor S.

                    spikes_per_state[S] += list(
                        simulation_data["E_spk_trains"][neur_id] / second
                    )

        for I, I_neur_ids in simulation_data["sFSA"][
            "I"
        ].items():  # for each neuron in an attractor representing an input.
            ...
            for neur_id in simulation_data[
                ...
                if (
                    neur_id in I_neur_ids
                ):  # save spikes if neuron belongs to attractor S.

                    spikes_per_state[I] += list(
                        simulation_data["E_spk_trains"][neur_id] / second
                    )
        ...
        S_winning = self.__getMostActiveAttractor(
            spikes_per_state, (0, 0.1), attractor_size
        )  # starting state.
        state_sequence.append(S_winning)

        _input_twindow = (
            simulation_data["input_twindow"][0] + simulation_data["input_twindow"][1]
        )
        ...
        if len(simulation_data["sim_t"]) > 0:
            ...
            for i in range(len(simulation_data["input_sequence"])):
                ...
                if i == len(simulation_data["input_sequence"]) - 1:
                    ...
                    t_start = (
                        simulation_data["sim_t"][-1] - 0.7
                    )  # look back .7s before an input token time window.
                    t_end = simulation_data["sim_t"][-1]

                else:

                    t_end = _input_twindow * (i + 1) + (
                        simulation_data["start_twindow"][0]
                        + simulation_data["start_twindow"][1]
                    )
                    t_start = t_end - 0.7

                S_winning = self.__getMostActiveAttractor(
                    spikes_per_state, (t_start, t_end), attractor_size
                )
    ...
    def plotSfsaNetwork(self, title_addition=""):
        ...
        ax2.hlines(
            y=self.sFSA_sim_data["thr_GO_state"],
            xmin=0,
            xmax=int(round(self.sFSA_sim_data["sim_t"][-1])) + 1,
            colors="b",
            linestyles="dashed",
            lw=0.75,
            ls="-.",
        ...
        ax1.set_xticks(
            np.arange(0, int(round(self.sFSA_sim_data["sim_t"][-1])) + 0.5, 0.5)
        )
    ...
    def makeSimulationFolder(self, sub_dir=""):
        ...
        if self.data_folder == "":
            ...
            if sub_dir != "":
                data_folder = os.path.abspath(
                    os.path.join(__file__, "../../", "results", sub_dir)
                )
                ...
                data_folder = os.path.abspath(
                    os.path.join(
                        __file__, "../../", "results", sub_dir, f"sFSA_{self.sFSA_id}"
                    )
                )
            else:
                data_folder = os.path.abspath(
                    os.path.join(__file__, "../../", "results", f"sFSA_{self.sFSA_id}")
                )
    ...
    def storeSfsa(self):
        ...
        self.__RCN.net.store(
            name=f"{self.sFSA_id}_initial_state",
