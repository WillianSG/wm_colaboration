...
class TelegramNotify:
        ...
        async with self.bot:
            ...
            try:
                for update in updates:
                    ...
                    ) if update.message.chat.id not in chat_ids else None
                return chat_ids
    ...
    async def _pin_message(self, message_id, chat_id):
        async with self.bot:
            msg = await self.bot.pin_chat_message(
    ...
    def send_messages(self, text):
        ...
        for chat_id in self.CHATIDs:
            messages.append(asyncio.run(self._send_message(text, chat_id)))
            ...
            try:
                last_updates.append(
                    (
                        chat.id,
                        int(
                            re.findall(r"\d+", chat.pinned_message.text_markdown_v2)[6]
                        ),
                        int(
                            re.findall(r"\d+", chat.pinned_message.text_markdown_v2)[7]
                        ),
                    )
                )
                self.unpin_all()
                max_up = max(last_updates, key=lambda x: x[1])
                self.edit_timestamped_messages(
                    f"*{max_up[1] + 1}/{max_up[2]}* Finished run. Score: {score}", msgs
                )
            except:
                pass
