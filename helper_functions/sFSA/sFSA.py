...
class sFSA:
    ...
    def __setStateTransitions(self):
        ...
        for tid, transition in self.__sfsa_state['T'].items():
            ...
            self.__RCN.set_synapses_A_2_B(A_ids = self.__sfsa_state['I'][_i], B_ids = self.__sfsa_state['S'][_x], weight = self.__args.w_trans*mV)

            # - Setting connections for state-input matching leading to next state.
            self.__RCN.set_synapses_A_2_GO(A_ids = self.__sfsa_state['I'][_i], GO_ids = self.__sfsa_state['S'][_y], weight = self.__args.w_acpt*mV)
            self.__RCN.set_synapses_A_2_GO(A_ids = self.__sfsa_state['S'][_x], GO_ids = self.__sfsa_state['S'][_y], weight = self.__args.w_acpt*mV)
    ...
    def __feedInputToken(self, input_token):
        ...
        self.__RCN.run_net(duration = 3.8)
