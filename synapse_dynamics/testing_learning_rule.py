...
Function:
- Script sets a plastic synapse operating under the rule _plasticity_rule_ between a pair of pre and postsynaptic neurons spiking with Poisson (normally distributed) spike trains, with firing frequencies of _pre_rate_ (Hz) and _post_rate_ (Hz), respectively.
- The code runs <num_sim> independent simulations of 1 second each. The state variables associated with the weight and calcium traces are saved tracked during each simulation and averaged out to build plots meant to show the behavior of the aforementioned variables during neuron activity.
...
Script output:
- The output of the script will appear in a folder called **synapse_results** one level up where the script resides (learning_rule/synapse_results). Grey curves represent data from each of the independent simulations, whereas colored thick curves represent the average of all <num_sim> simulations.
"""
import setuptools
...
f2_ax2.pie([pot_perc, dep_perc],
           labels=['+ (' + str(pot_avg_perc_change) + '%)', '- (' + str(dep_avg_perc_change) + '%)'], autopct='%1.1f%%',
