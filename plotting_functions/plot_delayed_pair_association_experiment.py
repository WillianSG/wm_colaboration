...
for folder in folders:
    ...
    for neuron_id in data['attractor_A']:
        if neuron_id in E_spk_trains.keys():
            ...
            ax1.scatter(spikes, y, c='b', s=5, label = 'A1', marker = '|')
    ...
    for neuron_id in data['attractor_B']:
        if neuron_id in E_spk_trains.keys():
            spikes = E_spk_trains[neuron_id]
            y = np.ones_like(spikes) * neuron_id
            ax1.scatter(spikes, y, c='r', s=5, label = 'A2', marker = '|')
    ...
    for neuron_id in data['attractor_C']:
        if neuron_id in E_spk_trains.keys():
            spikes = E_spk_trains[neuron_id]
            y = np.ones_like(spikes) * neuron_id
            ax1.scatter(spikes, y, c='g', s=5, label = 'A3', marker = '|')
    ...
    ax3.bar( input_e_t_hist_edgs[ :-1 ], input_e_t_hist_fr,
        input_e_t_hist_bin_widths,
        edgecolor='w',
        color='darkred',
        linewidth=1.0 )

    ax3.set_ylabel('inh pop [Hz]')
