...
for folder in folders:
    ...
    filepath = f'D://A_PhD//GitHub//wm_colaboration//results//RCN_FSA//{folder}//BA_15_GS_100_W_10_I_20'
    ...
    states_colors = ['mediumslateblue', 'magenta', 'purple', 'b', 'r', 'g', 'darkred', 'orange']
    ...
    for _s, neur_IDs in data['sFSA']['S'].items():
        ...
        for neuron_id in neur_IDs:
            if neuron_id in E_spk_trains.keys():
                ...
                ax1.scatter(spikes, y, c=states_colors[_aux_c], s=2.5, marker = '|')
    ...
    for _s, neur_IDs in data['sFSA']['I'].items():
        ...
        for neuron_id in neur_IDs:
            if neuron_id in E_spk_trains.keys():
                spikes = E_spk_trains[neuron_id]
                y = np.ones_like(spikes) * neuron_id
                ax1.scatter(spikes, y, c='k', s=2.5, marker = '|')
    ...
    # ax1.set_yticks(np.arange(0, data['sFSA']['N_e'], 50))
