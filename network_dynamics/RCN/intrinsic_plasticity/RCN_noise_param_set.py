...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Parallel sweep for adding fixed amount of noise to sets of parameters of RCN model with STSP and intrinsic plasticity.  '
    ...
    parser.add_argument('-param_set', '--parameter_set', type=str, default=[], help='Set of parameters to be varied')
    ...
    if args.parameter_distribution == 'grid':
        ...
        print(
            f'Sampling {args.num_samples} parameters from grid distribution and validating them {args.cross_validation} times')
    elif args.parameter_distribution == 'gaussian':
        param_dist = np.random.normal
        print(
            f'Sampling {args.num_samples} parameters from gaussian distribution and validating them {args.cross_validation} times')
    ...
    param_grid = {
        'background_activity': param_dist(args.background_activity[0], args.background_activity[1],
                                          args.num_samples) if len(
            args.background_activity) > 1 else args.background_activity,
        'i_e_weight': param_dist(args.i_e_weight[0], args.i_e_weight[1], args.num_samples) if len(
            args.i_e_weight) > 1 else args.i_e_weight,
        'e_i_weight': param_dist(args.e_i_weight[0], args.e_i_weight[1], args.num_samples) if len(
            args.e_i_weight) > 1 else args.e_i_weight,
        'e_e_max_weight': param_dist(args.e_e_max_weight[0], args.e_e_max_weight[1], args.num_samples) if len(
            args.e_e_max_weight) > 1 else args.e_e_max_weight,
        'i_frequency': param_dist(args.i_frequency[0], args.i_frequency[1], args.num_samples) if len(
            args.i_frequency) > 1 else args.i_frequency,
        'cue_percentage': param_dist(args.cue_percentage[0], args.cue_percentage[1], args.num_samples) if len(
            args.cue_percentage) > 1 else args.cue_percentage,
        'cue_length': param_dist(args.cue_length[0], args.cue_length[1], args.num_samples) if len(
            args.cue_length) > 1 else args.cue_length,
        'num_attractors': param_dist(args.num_attractors[0], args.num_attractors[1], args.num_samples) if len(
            args.num_attractors) > 1 else args.num_attractors,
        'num_cues': param_dist(args.num_cues[0], args.num_cues[1], args.num_samples) if len(
            args.num_cues) > 1 else args.num_cues,
        'attractor_size': param_dist(args.attractor_size[0], args.attractor_size[1], args.num_samples) if len(
            args.attractor_size) > 1 else args.attractor_size,
        'network_size': param_dist(args.network_size[0], args.network_size[1], args.num_samples) if len(
